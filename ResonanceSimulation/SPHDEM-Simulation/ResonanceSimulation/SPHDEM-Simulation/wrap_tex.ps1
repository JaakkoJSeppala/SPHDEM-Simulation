# PowerShell script to hard-wrap all lines in a LaTeX file to 80 characters using rewrap-cli
# Requires Node.js and rewrap-cli (install with: npm install -g rewrap-cli)

param(
    [string]$FilePath = "gradu.tex"
)

if (-not (Get-Command rewrap-cli -ErrorAction SilentlyContinue)) {
    Write-Host "rewrap-cli not found. Installing globally..."
    npm install -g rewrap-cli
}
...
Write-Host "Wrapping $FilePath to 80 columns..."
rewrap-cli $FilePath --column 80
Write-Host "Done."
...
\type{tutkielmapohjan esimerkki ja käsikirja} 
...
Kommentteja, parannusehdotuksia ja bugiraportteja voit lähettää
Gitlab-repositorioon
(\url{https://gitlab.jyu.fi/latex-thesis-classes/ttl-gradu}).
